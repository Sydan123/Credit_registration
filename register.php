...
<style type="text/css">
	...
	.wrap-login101 {
		...
		background: #9152f8;
		background: -webkit-linear-gradient(top, #0080ff, rgb(179, 215, 255));
	}
.login101-form-btn {
		...
		<div class="wrap-login101">
			<form class="" action="http://localhost/dangkyhoc/xulydangky.php" method="POST">
				<span class="">
					<div align="center">	
						<span class="" >
							<a href=""><img src="img/logomdc.png"></a>
						</span>
					</div>
				</span>
					...
					<div class="gt" data-validate = "">
						 ...
						 <?php
							...
							$sql = "SELECT lop, tenkhoa FROM lophoc INNER JOIN khoa ON lophoc.makhoa = khoa.makhoa ORDER BY tenkhoa";
							...
							while ($row = mysqli_fetch_array($kq)) {
								...
								// Hiển thị tên khoa một lần
								if ($khoa !== $previous_khoa) {
									echo "<optgroup label='" . $khoa . "'>";
								}

								echo "<option value=" . $row['lop'] . ">" . $row['lop'] . "</option>";

								// Cập nhật giá trị của biến $previous_khoa
								$previous_khoa = $khoa;
							}

							// Đóng thẻ optgroup cuối cùng nếu cần
							if (!is_null($previous_khoa)) {
								echo "</optgroup>";
							}
							?>
					...
					<div style="color: white; margin: 2px;">Email:</div>
					<div class="wrap-input100 validate-input" data-validate = "Enter username">
					...
					<div class="text-center p-t-90">
						<p class="" style="color: yellow">Bạn đã có tài khoản.
